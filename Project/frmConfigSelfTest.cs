...
namespace DefaultNS
{
    public partial class frmConfigSelfTest : Form
    {
        public frmConfigSelfTest()
        {
            InitializeComponent();
        }
        ...
        private void frmConfig_Load(object sender, EventArgs e)
        {
            ...
            txtBoardType.Text = Conv.atos(SQLite.ExecuteScalar("SELECT ItemValue FROM tbl_common_settings WHERE ItemName='BoardType'"));
            ...
            txtMesCOMMPort.Text = Conv.atos(SQLite.ExecuteScalar("SELECT ItemValue FROM tbl_common_settings WHERE ItemName='MesCOMMPort'"));
            txtDUTStaticIP.Text = Conv.atos(SQLite.ExecuteScalar("SELECT ItemValue FROM tbl_common_settings WHERE ItemName='DUTStaticIP'"));
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            SQLite.ExecuteNonQuery("UPDATE tbl_common_settings SET ItemValue='" + txtBoardType.Text + "' WHERE ItemName='BoardType'");
            ...
            SQLite.ExecuteNonQuery("UPDATE tbl_common_settings SET ItemValue='" + txtDUTStaticIP.Text + "' WHERE ItemName='DUTStaticIP'");
            SQLite.ExecuteNonQuery("UPDATE tbl_common_settings SET ItemValue='" + txtMesCOMMPort.Text + "' WHERE ItemName='MesCOMMPort'");
        ...
        private void btnTestCOM_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                serialPort1.PortName = txtMesCOMMPort.Text;
                serialPort1.Open();
                MessageBox.Show("Kiểm tra '" + txtMesCOMMPort.Text + "' thành công!");
                serialPort1.Close();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Lỗi thao tác mở cổng COMM");
            }
        }
    }
}
