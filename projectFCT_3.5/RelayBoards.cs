    ...
    public partial class RelayBoards : UserControl
    {
        ...
        public void Start(string port_name)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(ex.Message, "Lỗi kết nối với RELAYSBOARD", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                portCOMM.Close();
        ...
        void COMMProcess()
        {
            ...
            try
            {
                ...
                {
                    while (command_busy) Conv.delay_ms(1); //Chờ cho lệnh đọc kết thúc
                    thread_busy = true;
                    commOK = false;
                    try
                    {
                        ...
                        else
                        {
                            ...
                            Thread.Sleep(100 + m_Board[board_index].regLength * 2);
        ...
        public bool SetAllState(bool state)
        {
            ...
            for (int i = 0; i < m_Board.Count; i++)
            {
                ...
                {
                    if (ExecuteCommand(Conv.bytesToString(m_Board[i].CMD_CLEARALL_DO), 100) == null) retOK = false;
                }
            }
            return retOK;
        }
            ...
            try
            {
                ...
                for (int board_index = 0; board_index < m_Board.Count; board_index++)
                {
                    if (m_Board[board_index].CMD_READALL_DI != null)
                    {
                        ...
                        Conv.delay_ms(80 + m_Board[board_index].regLength * 2);
                        rx_len = portCOMM.BytesToRead;
                        if (rx_len == m_Board[board_index].regLength * 2 + 5)
                        {
                            byte[] rxBuff = new byte[rx_len];
